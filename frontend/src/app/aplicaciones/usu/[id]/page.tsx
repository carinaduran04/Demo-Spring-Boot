...
const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    const usuarioToSend = {
      ...usuario,
      userTypeId: usuario.userTypeId ? Number(usuario.userTypeId) : undefined,
    };
    ...
    if (!response.ok) {
      ...
      throw new Error("Error al guardar usuario: " + resultText);
    }
  ...
  showModalDialog(
    ...
    async () => {
      try {
        ...
        if (!res.ok) {
          console.error("Error del backend:", resultText);
          throw new Error("Error al eliminar usuario: " + resultText);
        }
       ...
       <form id="usuarioForm" onSubmit={onSubmit} className="grid grid-cols-12 gap-6">
          <div className="col-span-12 border rounded-lg p-6 bg-gray-50">
            ...
            <div className="grid grid-cols-6 gap-4">
              ...
              <InputField label="Compañía" value={usuario.company || ""} readOnly={!isEditing} onChange={(v) => handleChange("company", v)} />
              <InputField label="Tipo de Usuario" value={usuario.userTypeId?.toString() || ""} readOnly={!isEditing} onChange={(v) => handleChange("userTypeId", v)} />
            </div>
          </div>
        </form>
      </div>
    ...
    <div className={className}>
      ...
      <div className="relative">
        <input
          ...
          className={`w-full border border-green-600 rounded p-2 pr-10 text-sm ${
            readOnly ? "bg-gray-100" : "bg-white"
          }`}
          value={value || ""}
          readOnly={readOnly}
        ...
        {!readOnly && (
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 text-green-700 text-xs font-semibold"
          >

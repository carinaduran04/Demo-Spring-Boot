...
export default function UsuarioVerMas() {
 ...
 try {
      const usuarioToSend = {
      ...usuario,
      appointmentUserType: usuario.userTypeId ? { userTypeId: Number(usuario.userTypeId) } : null,
...
function InputPasswordField({ label, value, onChange, readOnly, className = "col-span-3" }: { label: string; value?: string; onChange: (val: string) => void; readOnly: boolean; className?: string }) {

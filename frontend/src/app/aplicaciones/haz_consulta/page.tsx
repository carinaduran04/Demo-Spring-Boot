...
export default function ConsultaForm() {
  ...
  const handleCancel = () => {
    if (!hasChanges) {
      console.log("No hay cambios, redirigiendo directamente...");
    ...
    showModalDialog(
      ...
      () => {
        console.log("Usuario confirmó cancelación, redirigiendo...");
        router.push("/aplicaciones/consulta");
      },
      () => {
        console.log("Usuario canceló la salida, quedando en el formulario");
      }
    );
  };

  const formatPhone = (input: string) => {
    const digits = input.replace(/\D/g, ""); // eliminar todo lo que no sea número

    // RD: códigos 809, 829, 849 + 7 dígitos
    if (/^(809|829|849)\d{7}$/.test(digits)) {
      return digits.replace(/(\d{3})(\d{3})(\d{4})/, "$1-$2-$3");
    }

    // US: 10 dígitos cualquier combinación
    if (/^\d{10}$/.test(digits)) {
      return digits.replace(/(\d{3})(\d{3})(\d{4})/, "$1-$2-$3");
    }
...
const handlePhoneChange = (value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!loggedUser) {
      showModalDialog("No hay usuario logueado");
      return;
    }
    ...
    if (!formattedPhone) {
      showModalDialog("Número de teléfono inválido. Debe ser RD o US (10 dígitos).");
      return;
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChangeValue("direccion", e.target.value)
                    }
                    required
                    className="w-full px-3 py-2 border border-gray-400 rounded focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChangeValue("ciudad", e.target.value)
                    }
                    required
                    className="w-full px-3 py-2 border border-gray-400 rounded focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleChangeValue("tipoConsulta", e.target.value)
                    }
                    required
                    className="w-full px-3 py-2 border border-gray-400 rounded focus:outline-none focus:ring-2 focus:ring-green-500"
                  >
